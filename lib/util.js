...
module.exports =  {
    ...
    exsits: fset.existsSync || PATH.existsSync,
    ...
    parseConfig: function(dir) {
        let content = fset.readFileSync(PATH.resolve(dir, 'config.json')).toString();
        let config;
    ...
    renderLists: function(dir) {
        ...
        me.eachSrcFile(srcDir, (path, s) => {
            ...
            info.url = '/archives/' + me.replaceExtname(path.slice(srcDir.length + 1));
    ...
    renderPost: function(dir, path) {
        ...
        html = me.renderFile(PATH.resolve(dir, '_layout', info.layout + '.ejs'), {
            config: me.parseConfig(dir),
            post: info,
            filename: PATH.resolve(dir, '_layout', 'widget', info.layout + '.ejs')
        });
