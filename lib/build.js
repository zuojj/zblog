...
const prism        = require('./prism-plugins/line-numbers');
const md           = new markdownIt({
    html: true,
    langPrefix: 'language-',
    highlight: function(str, lang) {
        lang = (lang || 'html').toLowerCase();
        let langObj = prism.languages[lang] || prism.languages['html'];

        return '<pre class="line-numbers language-'+ lang +'"><code class="language-' + lang +'">' + prism.highlight_linenum(str, langObj, lang) + '</code></pre>';
    }
});
...
module.exports = function(dir, options) {
    ...
    srcDir = PATH.resolve('_archives');

    !util.exsits(util.tmpPath) && util.mkdir(util.tmpPath);
    ...
    function parse_markdown(path) {
        ...
        mdInfo = markdown[0];
        mdContent = markdown.slice(1).join('---\n');
        ...
        page.thumb = page.thumb || util.randomThumb();
    ...
    function generate_archive(page) {
        ...
        html = ejs.render(util.read(PATH.resolve('_layout', page.layout + '.ejs')), {
            config: config,
            page: page,
            filename: PATH.resolve('_layout', 'widget', page.layout + '.ejs')
        });
    ...
    function generate_archives(archives) {
        ...
        html = ejs.render(util.read(PATH.resolve('_layout', 'archives.ejs')), {
            config: config,
            archives: archives,
            filename: PATH.resolve('_layout', 'widget', 'archives.ejs')
        });
    ...
    function generate_index(pages) {
        ...
        html = ejs.render(util.read(PATH.resolve('_layout', 'index.ejs')), {
            config: config,
            pages: pages,
            filename: PATH.resolve('_layout', 'widget', 'index.ejs')
        });
    ...
    function generate_categories(categories) {
        ...
        html = ejs.render(util.read(PATH.resolve('_layout', 'categories.ejs')), {
            config: config,
            categories: categories,
            filename: PATH.resolve('_layout', 'widget', 'categories.ejs')
        });
    ...
    function generate_tags(tags) {
        ...
        html = ejs.render(util.read(PATH.resolve('_layout', 'tags.ejs')), {
            config: config,
            tags: tags,
            filename: PATH.resolve('_layout', 'widget', 'tags.ejs')
        });
    ...
    function generate_about() {
        ...
        html = ejs.render(util.read(PATH.resolve('_layout', 'about.ejs')), {
            config: config,
            filename: PATH.resolve('_layout', 'widget', 'about.ejs')
        });
    ...
    function compile_static() {
        let srcPath = PATH.resolve('static');
        ...
        rd.eachFileFilterSync(srcPath, /.*/, (path, s) => {
            ...
            if(/\.(?:scss|sass)$/.test(ext)) {
                if(name.indexOf('_') !== 0) {
                    ...
                    if(config.compile.css.useUglify) {
                ...
                if(/\.min.js/.test(ext)) {
                    ...
                    if(config.compile.js.useUglify) {
